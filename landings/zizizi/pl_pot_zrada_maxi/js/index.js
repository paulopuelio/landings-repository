...
// Змінні для першого таймера (оригінальний)
var time = 600;
var intr;

// Змінні для другого таймера
var time2 = 600;
var intr2;

// Оригінальна функція для першого таймера
function start_timer() {
    intr = setInterval(tick, 1000);
}

// Нова функція для другого таймера
function start_timer2() {
    intr2 = setInterval(tick2, 1000);
}

// Функція для запуску обох таймерів
function start_both_timers() {
    start_timer();
    start_timer2();
}

// Оригінальна функція тіку для першого таймера
function tick() {
    time = time - 1;
...
// Нова функція тіку для другого таймера
function tick2() {
    time2 = time2 - 1;
    var mins = Math.floor(time2 / 60);
    var secs = time2 - mins * 60;
    if (mins == 0 && secs == 0) {
        clearInterval(intr2);
    }
    secs = secs >= 10 ? secs : '0' + secs;
...
// Запуск обох таймерів при завантаженні сторінки
$(document).ready(function() {
    start_both_timers();
});